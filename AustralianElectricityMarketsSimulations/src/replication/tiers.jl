...
Works around a gap in `PowerSimulations.jl` `Sv2Ox` (v0.34.2, the installed version): dual
registration for `NetworkModel` falls back to a generic method that keys duals by `PSY.ACBus`,
correct for nodal (PTDF/DC) network models — but `AreaBalancePowerModel`'s own
`CopperPlateBalanceConstraint` is `PSY.Area`-keyed (`network_models/area_balance_model.jl`), so
that fallback registers a dual container PSI can never fill. Whenever `ThermalBasicUnitCommitment`
makes the problem a MILP, `calculate_dual_variables!` then crashes recomputing duals ("constraint
CopperPlateBalanceConstraint__ACBus is not stored") — reproduced empirically against the mock
fixture; `CopperPlatePowerModel` (T0) is unaffected because it already has its own
`System`-keyed overload. This adds the missing `Area`-keyed one, mirroring that existing pattern
exactly (same `add_constraints!` call site, `network_models/area_balance_model.jl:1-21`) rather
than computing anything by hand — no shadow JuMP model, just the correct dual container shape.
"""
