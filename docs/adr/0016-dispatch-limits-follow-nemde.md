...
supply `DISPATCHLOAD` data for units nobody dispatched. This applies to `EnergyReservoirStorage`
as well as the three generator types.

### A battery's two ceilings are raised on the net axis, at build

A battery has no single `AVAILABILITY`, so the ramp-down-floor rule cannot be folded into a
`set_nem_dispatch_limits!` series the way it is for a generator. It is applied instead where the
formulation reads the per-direction ceilings, in
`AustralianElectricityMarketsSimulations/src/nem_dispatch_storage.jl`'s
`_storage_dispatch_ceilings`: a net ramp-down floor above the generation-side ceiling
(`get_storage_energy_max_avail`'s `gen`) raises that ceiling to the floor; a net ramp-up ceiling
below the negative of the load-side ceiling (`-load`) raises the load ceiling to match.
`_check_storage_dispatch_envelope` compares the net ramp floor/ceiling directly against the
device's own physical rating, not the raised availability ceiling, so a `MAXAVAIL` bid above
