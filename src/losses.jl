...
`model`'s loss factor at `flow` MW, given `demand` (`REGIONID => MW`). A region named in
`model.demand_coefficients` but absent from `demand` contributes nothing and is reported by
[`interconnector_loss_models`](@ref)'s caller, not here.
"""
...
The `- 1` is because `LOSSCONSTANT` is a loss *factor*, quoted relative to unity.
"""
...
Throws an `ArgumentError` when `model` carries fewer than two breakpoints: a single point
defines no segment, and silently returning an empty vector would present a lossless
interconnector as a modelled one.
"""
...
Differs from [`read_interconnectors`](@ref), which resolves to whatever version is latest
overall — a replication of a historical interval needs the version NEMDE itself held at the
time.

Throws an `ArgumentError` when either table isn't cached at all.
"""
...
An interconnector with loss parameters but no `LOSSMODEL` breakpoints is skipped and reported in
one summary `@warn`, never silently: without breakpoints its curve cannot be linearised, and
including it with an empty segment list would model it as lossless.

Throws an `ArgumentError` when no interconnector survives, since an empty result is
indistinguishable from a genuine "nothing to model" answer.
"""
