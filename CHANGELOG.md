...
- **`T0CopperPlate`/`T1Interconnected` fidelity tiers restored** (`AustralianElectricityMarketsSimulations/src/replication/tiers.jl`): `build_template`/`solve_interval`/`IntervalResult` wrap `docs/literate/economic_dispatch.jl` (T0) and `interchanges.jl` (T1) into real `PowerSimulations.jl` `DecisionModel` problems, restored from the pre-`v0.2` replication work.
  The `PSI.add_constraint_dual!` type-piracy workaround for `AreaBalancePowerModel` (still an open gap through PSI 0.38.3 — confirmed empirically) now lives in its own quarantined `src/psi_compat.jl` instead of inside `tiers.jl`.
  The "Fidelity tiers" test suite is rewritten against `PowerSystemCaseBuilder`'s `augmented_pscb_system()` fixture (supply-adequate by construction) instead of the old unphysical mock, building/solving `build_template`'s `ProblemTemplate` directly via PSI rather than through `solve_interval` — `solve_interval`'s `set_demand!`-based NEMWEB seeding only matches `nem_system`-built `PowerLoad` names and is incompatible with the PSCB fixture's own bus-based names.
