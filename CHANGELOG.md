...
- **Per-unit conversion helpers** (`AustralianElectricityMarketsSimulations/src/per_unit.jl`): `mw_to_pu`/`pu_to_mw` convert between natural MW and per-unit of a `base_power::Float64` or a `PSI.OptimizationContainer`'s own base power; `price_to_pu_coefficient` turns a `$/MWh` price into an objective coefficient on a per-unit dispatch variable, accounting for the five-minute dispatch interval; `dimensionless_factor` is a documented no-op for multipliers, such as a `GenericConstraint` term's `factor`, that must never be scaled. `DISPATCH_INTERVAL_HOURS` moved here from `src/replication/preprocessing.jl`, which now uses the single definition. This is the single point where later formulation PRs convert between market units and PSI's per-unit basis.
