...
- **`has_nem_dispatch_limits(device, formulation)`**, which reports whether a component carries every time series a formulation reads, and `set_nem_dispatch_models!(...; skip_uncovered = true)`, which excludes uncovered components from the device models behind a warning instead of failing the build. This is the sim-side counterpart to `set_nem_dispatch_limits!(...; allow_missing_ramp_rates = true)`: without it, proceeding with a partial subset in the data layer still could not be built. Participation is now decided by the formulation's full registered series set rather than by probing `"ramp_up_rate"` alone.
