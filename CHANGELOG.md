...
- **`T0CopperPlate`/`T1Interconnected`/`solve_interval`** (`AustralianElectricityMarketsSimulations`): Wraps `docs/literate/economic_dispatch.jl`'s and `interchanges.jl`'s `ProblemTemplate`s into per-interval `PowerSimulations.jl` `DecisionModel` solves, reading dispatch and prices back out via `PSI.read_variable`/`PSI.read_dual` — every variable and constraint comes from PSI's own machinery, no hand-rolled `JuMP.Model`. Also adds a `PSI.add_constraint_dual!` method for `NetworkModel{AreaBalancePowerModel}`, working around a gap in the installed `PowerSimulations.jl` (`Sv2Ox`, v0.34.2) where dual computation for that network model crashes once `ThermalBasicUnitCommitment` makes the problem a MILP.
...
- **`add_fcas_variables!`** (`AustralianElectricityMarketsSimulations`): Post-build augmentation of a `PowerSimulations.jl` `DecisionModel`'s JuMP model (via `PSI.get_jump_model`) — adds one non-negative decision variable per (unit, FCAS service, timestep), bounded above by that unit's `max_avail`, for every unit carrying a matching `"fcas_trapezium_<SERVICE>"` series. Lives in `AustralianElectricityMarketsSimulations` to keep the root package free of any JuMP/PowerSimulations dependency. `t` indexes the model's own dispatch timesteps (`PSI.get_timestamps`), each row looked up in the trapezium series by absolute timestamp rather than by position — a model timestep with no matching row throws rather than silently misaligning. Decremental/load-direction (`EnergyReservoirStorage` `"_decremental"`) variables remain deferred to a later milestone.
