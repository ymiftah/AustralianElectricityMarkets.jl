...
- **`add_fcas_variables!`** (`AustralianElectricityMarketsSimulations`): Post-build augmentation of a `PowerSimulations.jl` `DecisionModel`'s JuMP model (via `PSI.get_jump_model`) — adds one non-negative decision variable per (unit, FCAS service, timestep), bounded above by that unit's `max_avail`, for every unit carrying a matching `"fcas_trapezium_<SERVICE>"` series. Lives in `AustralianElectricityMarketsSimulations` to keep the root package free of any JuMP/PowerSimulations dependency. Two caveats: the variables index the trapezium series's own timesteps rather than the model's dispatch timesteps — the two grids are currently disjoint — and decremental/load-direction (`EnergyReservoirStorage` `"_decremental"`) variables are deferred to a later milestone.
