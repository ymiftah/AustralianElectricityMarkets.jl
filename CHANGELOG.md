...
- **`GenericConstraint` as a `PowerSimulations.jl` service, energy terms only** (`AustralianElectricityMarketsSimulations/src/services/nem_constraints.jl`): `NEMGenericConstraint <: PSI.AbstractServiceFormulation` drives `GenericConstraint` (now `<: PSY.Service`) through PSI's `ServiceModel` machinery — `NEMConstraintLHS`/`NEMConstraintLimit`/`NEMConstraintRHSParameter`, per-term-type `add_to_expression!` (`InterconnectorTerm`, and `UnitTerm`/`RegionTerm` restricted to `bid_type == ENERGY`; FCAS terms are a later task), and sense-dispatching `add_constraints!` that honours the `"invoked"` series.
  ...
  `AustralianElectricityMarketsSimulations/src/psi_compat.jl` gained one more quarantined override, a no-op `PSI._modify_device_model!` for this formulation — PSI's private `_add_services_to_device_model!` calls it unconditionally and there is no fallback method.
