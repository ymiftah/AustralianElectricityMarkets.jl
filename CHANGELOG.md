...
- **Unit physical bid parameters and Marginal Loss Factors**: energy-path `ROCUP`/`ROCDOWN` now flow through `read_bids`/`_read_energy_ramp_rates`, and `set_bid_ramp_rates!` attaches each `ThermalStandard`/`HydroDispatch` generator's effective `min(bid, registered)` ramp rate as a `"bid_ramp_rate"` time series (falling back to the registered rate when a unit doesn't bid one that interval); `set_bid_minimum_load!` attaches the previously-read-but-unused `BIDDAYOFFER_D.MINIMUMLOAD` as a `"bid_minimum_load"` series; and new `read_marginal_loss_factors`/`set_marginal_loss_factors!` resolve `DUDETAILSUMMARY.TRANSMISSIONLOSSFACTOR` over a date window (reusing `read_constraint_terms`'s versioning pattern) and carry it onto each unit's `ext["transmission_loss_factor"]` for a later PR's regional energy balance to consume.
- **Interconnector loss model** (`src/losses.jl`): `InterconnectorLossModel`, three readers `read_interconnector_loss_breakpoints`/`read_interconnector_demand_coefficients`/`read_interconnector_loss_parameters`, and `interconnector_loss_models` assembler. NEMDE models losses as quadratic in flow with demand-dependent linear coefficients; `loss_factor` evaluates it, `interconnector_losses` integrates it, and `loss_segments` linearises on `LOSSMODEL`'s `MWBREAKPOINT`s as chord slopes. Readers are version-resolved on `EFFECTIVEDATE`/`VERSIONNO` as of a caller-supplied date, not `archive_month`, and throw `ArgumentError` naming the missing table when uncached.
- **`NEMInterconnectorLoss`** (`AustralianElectricityMarketsSimulations/src/devices/interconnector_losses.jl`): new `PSY.AreaInterchange` device formulation putting NEMDE's `InterconnectorLossModel` losses into the `AreaBalancePowerModel`/`AreaPTDFPowerModel` area balance, linearised on the model's own breakpoints as monotonically-increasing chord slopes (no SOS2/binary needed); loss models are supplied via the `DeviceModel`'s "loss_models" attribute, and an interconnector missing from it throws `ArgumentError` rather than being silently treated as lossless.
